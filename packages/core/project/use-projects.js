...
export function useProjects() {
    ...
    return useQuery({
        queryKey: ['projects'],
        queryFn: async () => {
            const raw = await api.getList('/api/v1/projects', token ?? undefined);
...
export function useProject(projectId) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            const raw = await api.get(`/api/v1/projects/${projectId}`, token ?? undefined);
            return parseWithFallback(z.object({ data: ProjectSchema }), raw, 
...
export function useCreateProject() {
    ...
    return useMutation({
        mutationFn: async (name) => {
            const raw = await api.post('/api/v1/projects', { name }, token ?? undefined);
